...
class RedCenterDriveOut(Mode):
    def __init__(self):
        #this is naming the mode, in this case "Drive Out"
        Mode.__init__(self, f"R-Center Drive Out")
        ...
        self.pathCmd = PoserSchemeCommand(AutonPoserSelected.R_CENTER_DRIVE)

    def getCmdGroup(self):        # Just return the path command normally, since we're only doing one path.
